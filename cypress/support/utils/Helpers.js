class Helpers {
  ...
  /**
 * For searchable MUI Autocomplete-style dropdowns — types into the filter
 * input to narrow the list down to a single match, then clicks it.
 */
  static selectSearchableDropdownOption(selectLocator, optionText, optionsContainerRole = "menu") {
    cy.log(`Opening searchable dropdown: ${selectLocator}`);
    cy.get(selectLocator)
      .should("be.visible")
      .and("not.have.class", "Mui-disabled")
      .click();

    cy.focused().should("match", "input").type(optionText, { delay: 50 });
